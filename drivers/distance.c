...
uint8_t _i2caddr = ADDRESS_DEFAULT;
...
int distancebegin() {
  ...
  cy_rslt_t rslt;
	...
	rslt = cyhal_gpio_init(P10_2, CYHAL_GPIO_DIR_OUTPUT,CYHAL_GPIO_DRIVE_OPENDRAINDRIVESLOW, false);
	if(rslt != CY_RSLT_SUCCESS)
  {
      printf("Error initializing distance pin\n\r");
  }

  write8(VL6180X_REG_SLAVE_DEVICE_ADDRESS, 0x30 & 0x7F);
  setAddress(0x30);
  uint8_t value = read8(VL6180X_REG_IDENTIFICATION_MODEL_ID);
  if (value != 0xB4) {
    printf("Error initializing distance sensor 1 (Addr 0x30)\n\r");
    return false;
  }
  ...
  cyhal_gpio_write(P10_2, 1);
  setAddress(0x29);
  value = read8(VL6180X_REG_IDENTIFICATION_MODEL_ID);
  if (value != 0xB4) {
    printf("Error initializing distance sensor 2 (Addr 0x29)\n\r");
    return false;
  }
  ...
  printf("Distance sensors initialized\n\r");
...
*/
/**************************************************************************/
uint8_t getAddress(void) { return _i2caddr; }
...
//   i2c_dev->read(buffer, 1);
  cyhal_i2c_master_write(&i2c_master_obj, _i2caddr, buffer, 2, 0, false);
  cyhal_i2c_master_read(&i2c_master_obj, _i2caddr, buffer, 1, 0, false);
  return buffer[0];
}
...
//   i2c_dev->read(buffer, 2);
  cyhal_i2c_master_write(&i2c_master_obj, _i2caddr, buffer, 2, 0, false);
  cyhal_i2c_master_read(&i2c_master_obj, _i2caddr, buffer, 2, 0, false);
...
//   i2c_dev->write(buffer, 3);
  cyhal_i2c_master_write(&i2c_master_obj, _i2caddr, buffer, 3, 0, false);
}
...
//   i2c_dev->write(buffer, 4);
  cyhal_i2c_master_write(&i2c_master_obj, _i2caddr, buffer, 4, 0, false);
}